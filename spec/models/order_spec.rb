...
describe Order, type: :model do
  ...
  describe "class methods" do
    before :each do
      ...
      @order_2 = @user.orders.create!(name: 'Meg', address: '123 Stang Ave', city: 'Hershey', state: 'PA', zip: 17033, status: 3)

      @order_2.item_orders.create!(item: @pull_toy, price: @pull_toy.price, quantity: 3)

      @order_3 = @user.orders.create!(name: 'Meg', address: '123 Stang Ave', city: 'Hershey', state: 'PA', zip: 17033, status: 1)
    ...
    it 'orders by status' do
      expect(@user.orders.order_by_status).to eq([@order_1, @order_3, @order_2])
    end
